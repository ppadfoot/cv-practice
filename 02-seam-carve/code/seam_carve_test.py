...
for filename in iglob(input_dir + '/*.png'):
    ...
    if mode == 'base':
        for orientation in ('horizontal', 'vertical'):
            if load(file) != seam_coords(seam_carve(img, orientation + ' shrink')[2]):
                print(orientation)
            else:
                TP += 1
    elif mode == 'full':
        for m in (None, msk):
            for direction in ('shrink', 'expand'):
                for orientation in ('horizontal', 'vertical'):
                    if load(file) != seam_coords(seam_carve(img, orientation + ' ' + direction, mask = m)[2]):
                        print(m is None, direction, orientation)
                    else:
                        TP += 1

    end = datetime.now()
    delta = end - beg
    print('time for {} is {}'.format(filename, delta.seconds))
    file.close()
